...
int Breadth_First_Search(std::vector<std::string>& grid, int y, int x, int y_direction, int x_direction) {
	...
	node_queue.push({ y-y_direction,x-x_direction,y_direction,x_direction });

	while (!node_queue.empty()){

		y = node_queue.front()[0] + node_queue.front()[2];
		x = node_queue.front()[1] + node_queue.front()[3];
		y_direction = node_queue.front()[2];
		x_direction = node_queue.front()[3];
		node_queue.pop();
...
int Count_Energized_Cells(const std::string& input_file_path) {
	...
	for (int i = 0; i < grid[0].size(); i++) {
		current = Breadth_First_Search(grid, 0, i, 1, 0);
		result = (result > current) ? result : current;
		current = Breadth_First_Search(grid, grid.size() - 1, i, -1, 0);
		result = (result > current) ? result : current;
	}

	for (int i = 0;i < grid.size(); i++) {
		current = Breadth_First_Search(grid, i, 0, 0, 1);
		result = (result > current) ? result : current;
		current = Breadth_First_Search(grid, i, grid[0].size()-1, 0, -1);
		result = (result > current) ? result : current;
	}
