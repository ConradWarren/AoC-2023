...
int Djikstra(const std::vector<std::string>& grid) {
	...
	while (!node_queue.empty()){
		...
		if (In_Range(temp_y, temp_x, grid.size(), grid[0].size()) && (cache.find({temp_y, temp_x, x_dir, y_dir, 1}) == cache.end() || cache[{temp_y, temp_x, x_dir, y_dir, 1}] > heat_loss + grid[temp_y][temp_x] - '0')) {
