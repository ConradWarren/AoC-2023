...
int Find_Axis_of_Reflection(std::vector<std::string>& grid) {
    ...
    std::vector<int> verticle_reflections(grid[0].size()-1, 0);
    std::vector<int> horizontal_reflections(grid.size() - 1, 0);
    int idx_1 = 0, idx_2 = 0;
    ...
    for (int i = 0; i < grid.size(); i++) {
        ...
        for (int j = 0; j < grid[0].size()-1; j++) {
            ...
            while (idx_1 >= 0 && idx_2 < grid[0].size()){
                ...
                if (grid[i][idx_1] != grid[i][idx_2]) verticle_reflections[j]++;
    ...
    for (int i = 0; i < verticle_reflections.size(); i++) {
        if (verticle_reflections[i] == 1) return i + 1;
    }
    ...
    for (int i = 0; i < grid[0].size(); i++) {
        ...
        for (int j = 0; j < grid.size()-1; j++) {
            ...
            while (idx_1 >= 0 && idx_2 < grid.size()){
                if (grid[idx_1][i] != grid[idx_2][i]) horizontal_reflections[j]++;
    ...
    for (int i = 0; i < horizontal_reflections.size(); i++) {
        if (horizontal_reflections[i] == 1) return 100 * (i + 1);
    }
